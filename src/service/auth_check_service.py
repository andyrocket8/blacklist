...
from .token_db_services import AdminTokensSetDBEntityService
from .token_db_services import AgentTokensSetDBEntityService
...
class AuthCheckService:
    ...
    async def check_token(self, token: UUID) -> AuthCheckResult:
        agent_db_srv_obj = AgentTokensSetDBEntityService(SetDbEntityStrAdapterUUID(RedisSetDbEntityAdapter(self._db)))
        is_authenticated_user = token in await agent_db_srv_obj.get_records()
        admin_db_srv_obj = AdminTokensSetDBEntityService(SetDbEntityStrAdapterUUID(RedisSetDbEntityAdapter(self._db)))
