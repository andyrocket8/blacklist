...
class AbstractSetDBEntityService(Generic[T]):
    ...
    async def fetch_records(self, records_count: int = 0, all_records: bool = True) -> AsyncGenerator[T, None]:
        """Generator function for fetching records"""
        current_record = 0
    ...
    async def get_records(self, records_count: int = 0, all_records: bool = True) -> list[T]:
        """
        Getting records from database. Return list
        """
        result = []
...
AbstractSetDBServiceType = TypeVar('AbstractSetDBServiceType', bound=AbstractSetDBEntityService)
