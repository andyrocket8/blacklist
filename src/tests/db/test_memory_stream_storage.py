...
async def test_memory_stream_storage(stocks_set_test_data: StocksTestDataSet):
    ...
    for test_record in stock_test_data:
        await memory_stream_adapter.save_by_timestamp(set_1, test_record.stock_data, test_record.stock_data.datetime)
    ...
    for test_set in stocks_set_test_data.check_filter_data_list:
        ...
        async for record in memory_stream_adapter.fetch_records(
            set_1, start_ts=test_set.start_date, end_ts=test_set.end_date
        ):
    ...
    async for record in memory_stream_adapter.fetch_records(set_1):
        records.append(SetDataRecord(set_1, record[0], record[1]))
    # add the same data to another set, with ordering as in list
    for test_record in stock_test_data:
        await memory_stream_adapter.save(set_2, test_record.stock_data)
    ...
    for storage_record, test_set_record in zip(
        [storage_record async for storage_record in memory_stream_adapter.fetch_records(set_2)], stock_test_data
    ):
        assert (
    ...
    for record_data in records:
        print(await memory_stream_adapter.read(record_data.set_id, record_data.timestamp))
    assert (await memory_stream_adapter.delete(set_1, map(lambda x: x.timestamp, records))) == len(
        stock_test_data
