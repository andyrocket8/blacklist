...
class RedisStreamDbAdapter(IStreamDbAdapter[str, str, str, str]):
    ...
    async def save_by_timestamp(self, stream_id: str, values: dict[str, str], timestamp: Optional[int] = None) -> str:
        try:
            return await self.__db.xadd(
                stream_id,
                # some mypy lint error patching here
    ...
    async def read(self, stream_id: str, timestamp_id: str) -> dict[str, str]:
        try:
            ...
            return {str(key): str(value) for key, value in values[1].items()} if values else dict()
        except RedisError as e:
            logging.error(f'Error while deleting from stream, stream ID: {stream_id}, details: {str(e)}')
            raise IStreamDbError from None
        ...
        try:
            ...
            while True:
                ...
                if len(values):
                    for value in values:
                        # transform to dict[str, str] here
                        yield str(value[0]), ({str(k): str(v) for k, v in value[1].items()} if value else dict())
